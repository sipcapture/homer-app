define(['app'], function(app) {
    ...
    /* settings */
    app.dashboard.widget('elasticchart', {
        title: 'Elasticsearch Charts',
        group: 'Charts',
        name: 'elasticchart',
        description: 'Displaycharts time',
        templateUrl: 'widgets/elasticchart/elasticchart.html',
        controller: 'elasticchartController',
        controllerAs: 'elasticchart',
        refresh: true,                
        sizeX: 1,
        sizeY: 1,
        config: {
            title: "Elasticchart Stats",
            chart: {},
            dataquery: {},
            panel: {
                queries:[]
        ...
        edit: {
            controller: 'elasticchartEditController',
            templateUrl: 'widgets/elasticchart/edit.html'
        },
        api: { }
    });

   /*SERVICE*/
   var injectElasticChartParams = ['$q', '$http', 'CONFIGURATION'];
   var elasticchartFactory = function($q, $http, CONFIGURATION) {
   ...
   elasticchartFactory.$inject = injectElasticChartParams;
   app.register.factory('elasticchartService', elasticchartFactory);


   /*** Controller ***/
   var injectElasticChartChartsParams = ['$scope', '$timeout', '$q', 'userProfile','$rootScope', 'eventbus', '$interval', '$uibModal', 'elasticchartService', 'EVENTS', 'DATASOURCES', 'CONFIGURATION'];
   var elasticChartController = function($scope, $timeout, $q, userProfile, $rootScope, eventbus, $interval, $uibModal, elasticchartService, EVENTS, DATASOURCES, CONFIGURATION) {
	...
	function findAndReplace(object, from, to, timezone) {
 		for (var x in object) {
			if (object.hasOwnProperty(x)) {
				if (typeof object[x] == 'object') {
					findAndReplace(object[x], from, to, timezone);
				}
				...
				else if(x == 'date_histogram') {

			                if(object[x].time_zone) {
						object[x].time_zone = timezone;
			                }

			                if(object[x].extended_bounds) {

						if(object[x].extended_bounds.min) {
							object[x].extended_bounds.min = from;	
						}

						if(object[x].extended_bounds.max) {
							object[x].extended_bounds.max = to;	
						}
			                }
				}
			}
        ...
        function queryBuilder(qvr) { 
	      ...
	      findAndReplace(query, timedate.from.getTime() - diff, timedate.to.getTime() - diff, "UTC");
        ...
        function loadNewChart(sipdata, query) {
                
            var config = $scope.config;
	    var rKey = {};

	    // console.log("S",sipdata);
	    // console.log("Q",query);

	    angular.forEach(query.aggs, function(tobj, tkey) {
			
			rKey[tkey] = tkey;
			if(tobj.aggs)						
			{	
			        console.log("INSIDE AGGS");
			        if(!rKey["aggs"]) rKey["aggs"] = {};
			        angular.forEach(tobj.aggs, function(o, k) {
			            rKey["aggs"][k] = k;
                                });
			}			
	    });
            ...
            if (config) {            
                ...
                if (config.chart.hasOwnProperty("library") && config.chart.library.value == "d3") {
                    ...
                    if (config.chart.type && config.chart.type.hasOwnProperty("value")) {
                            // console.log(sipdata);
                            elasticSearchD3Draw($scope, config.chart.type["value"], sipdata, query, rKey);
                    }
                } else if (config.chart.hasOwnProperty("library") && config.chart.library.value == "flot") {
                    $scope.flotEnabled = true;
                    $scope.chartHeight = config.chart.size.height;
                    $scope.chartWidth = config.chart.size.width;
                    if (config.chart.type && config.chart.type.hasOwnProperty("value")) elasticchartWdgt.flot.draw($scope, config.chart.type["value"], sipdata);
                }
            }
        };
        ...
        function loadDataChart(sipdata) {
            ...
            if (config) {            
                ...
                if (config.chart.hasOwnProperty("library") && config.chart.library.value == "d3") {
                    ...
                    $scope.chartWidth = config.chart.size.width;
                    if (config.chart.type && config.chart.type.hasOwnProperty("value")) 
                            elasticchartWdgt.flot.draw($scope, config.chart.type["value"], sipdata);
                }
            }
        };
        ...
        {      
            ...
            if(config.path && config.query) 
            {
                ...
                sipCaptureobjQuery = queryBuilder(config.query);                        
                sipCaptureobjParam.path = config.path;

                if(!angular.equals(sipCaptureobjQuery, {})) {
                    // console.log(sipCaptureobjQuery);                        
                    elasticchartService.get(config, sipCaptureobjParam.path, sipCaptureobjQuery).then(function(sdata) {

                            console.log("RELOADING",sdata);
                            // console.log("query",sipCaptureobjQuery);			    


                            loadNewChart(sdata, sipCaptureobjQuery);
                    }, function(sdata) {
        ...
        /**************************************** D3 FUNCTIONS ***********************/
        function elasticchartD3Data(data, query, rKey) {
                ...
                angular.forEach(data.aggregations, function(dataLoc, key)
                {
			angular.forEach(dataLoc.buckets, function(entry, kat){
        
	                        var name = "";
        	                var value = 0;

				// console.log("DDD", entry);
				// console.log("TTT", kat);

	                        //name = "counter";
				//name = entry["countername"]
        	                value = parseInt(entry['doc_count']);

                	        //if (names.indexOf(name) === -1) {names.push(name)}

        	                timefieldData = parseInt(entry['key'])/1000;
	                        ...
	                        */
	                        ...
	                        angular.forEach(rKey["aggs"], function(e, k){
	                                    // console.log("RR",k);
	                                    if(entry.hasOwnProperty(k))
	                                    {
	                                            angular.forEach(entry[k].buckets, function(b,v) 
	                                            {
	                                                    name = b['key'];
	                                                    // console.log("CC",name);
	                                                    value = parseInt(b['doc_count']);
	                                                    // console.log("V", value);
	                                                    // console.log("R", e);
	                                                    if (names.indexOf(name) === -1) { names.push(name);}
		...
		// console.log("TIMEFIELD", timefieldData);
        ...
        function elasticchartD3BarData(data, query, rKey) {
        ...
        function elasticchartD3PieData(data, query, rKey) {
	...
	/******************* CHECK OF TYPE *****************/
        ...
        function elasticSearchD3Draw ($scope, type, data, query, rKey) {
                // console.log(type);

                var customData;                

                if (type == "pie") {
                        customData = elasticchartD3PieData(data, query, rKey);                        
                        sipCaptureD3pieChart();
                } else if (type == "scatter") {
                       //customData = elasticchartWdgt.d3.scatterChart.data(customData);
                       customData = elasticchartD3Data(data, query, rKey);                        
                        ...
                        customData = elasticchartD3BarData(data, query, rKey);
        ...
        function sipCaptureD3PrepareData (type, data, query, rKey) {
                // console.log(type);
    ...
    elasticChartController.$inject = injectElasticChartChartsParams;
    app.register.controller('elasticchartController', elasticChartController);

    /* EDIT */
    var injectElasticchartEditSettingsParams = ['$scope', '$timeout', '$rootScope', '$uibModalInstance', 'config', '$http', 'userProfile', 'DATASOURCES', 'CONFIGURATION'];
    var elasticchartEditController = function($scope, $timeout, $rootScope, $uibModalInstance, config, $http, userProfile, DATASOURCES, CONFIGURATION) {
        ...
        $scope.updateDebugUrl = function() {
            ...
            try {
                ...
                //objQuery = elasticchartWdgt.query($scope, objQuery, userProfile);

                $scope.debug = "curl -v --cookie 'HEPICID=A12433443" + "' -X POST \\\n" + "-d '" + JSON.stringify(objQuery) + "' \\\n" + ' "' + window.location.protocol + "//" + window.location.host + "/" + url + '"\n';
        ...
        //console.log(elasticchartWdgt);

        //$scope.datasources = elasticchartWdgt.data.datasources.datasources;
        //$scope.metricsdatasources = elasticchartWdgt.data.metricsdatasources.datasources;
    ...
    elasticchartEditController.$inject = injectElasticchartEditSettingsParams;
    app.register.controller('elasticchartEditController', elasticchartEditController);

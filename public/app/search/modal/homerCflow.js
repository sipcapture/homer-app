			...
			setContext: function(id, dataSIP) {
				...
				for (var callid in dataSIP.callid)
				{
						...
						colorLine = this.getCallIDColor(callid);
		                                ...
		                                dataSeriesCallid[callid] = {
		                                        ...
		                                        markerSize: 2, 
		                                        divColor: {"color": colorLine},
		                                        lineColor: colorLine
                                                };
		                                this.dataCallid.push({name: callid, type: 'square', legendText: callid, markerColor: colorLine, markerSize: 2, lineColor: colorLine});
                		        }               
				}
				...
				angular.forEach(data['calldata'], function(value) {
					...
					if (value.message) {
						...
						} catch(err) { console.log('no message text'); }
					}

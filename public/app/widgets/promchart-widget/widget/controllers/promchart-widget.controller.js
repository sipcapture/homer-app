const promchartWidget = function($scope, $timeout, UserProfile, $rootScope, EventBus,
  ...
  log.initLocation('promchartWidet');
  const self = this;
  $scope.$state = $state;
  ...
  self.openSettings = function() {
    $uibModal.open({
      component: 'promchartWidgetSettings',
      resolve: {
        config: () => {
          return $scope.config;
        },
    ...
    if (config.dataquery && config.dataquery.data) {
      ...
      for (let i = 0; i < config.dataquery.data.length; i++) {
        ...
        if (dalias == 'prometheus') {
          ...
          hepiChartobjQuery.param.metrics = nData;
        }
      };
  ...
  function hepichartD3Data(data, scObject) {
    ...
    for (let each = 0; each < data.length; each++) {
        ...
        let valArray = entry['values'];                        
        
        for (let ev = 0; ev < valArray.length; ev++) {                          
              timefieldData = valArray[ev][0];
              timefields.push(timefieldData);
              let value = parseInt(valArray[ev][1]);          
  ...
  function hepichartD3BarData(data, scObject) {
    ...
    angular.forEach(data, function(dataLoc, key) {
        ...
        angular.forEach(dataLoc['values'], function(entry) {
